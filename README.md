...
EmotionAI 是一个基于 AstrBot 的高级情感智能交互插件，旨在为 AI 模拟一个多维度的、自主演变的内心世界。与传统的单一好感度系统不同，EmotionAI 提供了更丰富的情感维度和更智能的交互体验。
...
\- \*\*session\_based\*\*: 是否启用会话独立模式

\- \*\*favour\_min\*\*: 好感度最小值（达到此值将加入黑名单）

\- \*\*favour\_max\*\*: 好感度最大值

\- \*\*change\_min\*\*: 单次好感度减少最大值

\- \*\*change\_max\*\*: 单次好感度增加最大值

\- \*\*admin\_qq\_list\*\*: 管理员QQ号列表
...
A: 在插件配置中修改 `change\_min` 和 `change\_max` 参数。
